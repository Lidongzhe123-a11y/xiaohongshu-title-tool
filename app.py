...
def generate_signature(ak, sk, method, host, path, params):
...
def call_qianfan_api(messages):
    ...
    headers = {
        ...
        "Authorization": generate_signature(ak, sk, "POST", "aip.baidubce.com", "/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/completions", {})
    }
    data = {
        "model": "ERNIE-Speed",
        "messages": [{
    ...
    try:
        ...
        logging.error(f"SSL错误: {ssl_err}")
        raise Exception(f"SSL错误: {ssl_err}")
    except Exception as e:
...
def test_connection():
    def test_connection(domain, port=443):
        try:
            ...
            headers = {
                "Content-Type": "application/json",
                "Authorization": generate_signature(ak, sk, "POST", domain, "/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/completions", {})
            }
            data = {
                "model": "ERNIE-Speed",
                "messages": [{

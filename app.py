...
def test_connection():
    def test_connection(ip, port=443):
        try:
            result = subprocess.run(
                ["curl", "-I", f"https://{ip}:{port}", "--connect-timeout", "10"],
                capture_output=True,
                text=True,
                timeout=15
            )
            if result.returncode == 0:
                return f"Connection successful to {ip}:{port}\n{result.stdout}"
            else:
                return f"Connection failed to {ip}:{port}\n{result.stderr}"
        except subprocess.TimeoutExpired:
            return f"Connection timed out to {ip}:{port}"

    ipv4_address = "14.215.182.8"
    ipv6_address = "240e:ff:e020:934:0:ff:b0dc:3636"

    ipv4_result = test_connection(ipv4_address)
    ipv6_result = test_connection(ipv6_address)

    return f"IPv4 Connection Test:\n{ipv4_result}\n\nIPv6 Connection Test:\n{ipv6_result}"
